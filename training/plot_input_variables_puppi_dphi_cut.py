...
input_files=[]
for idx in range(17):
   inputfile="input/puppi_2018_GenJetIdx_dphi_cut/training_trees_pt10To100_puppi_106X_%i_2018.root" %idx
...
for x, eta_bin in enumerate(eta_bins):
   ...
   for input_file_name in input_files:
      ...
      for entry in range(sig_tree.GetEntries()): 
         ...
         Prompt_dR2Mean=getattr(sig_tree,"JetPuppi_puId_dR2Mean")
         Prompt_frac01=getattr(sig_tree,"JetPuppi_puId_frac01")
         Prompt_frac02=getattr(sig_tree,"JetPuppi_puId_frac02")
         Prompt_frac03=getattr(sig_tree,"JetPuppi_puId_frac03")
         Prompt_frac04=getattr(sig_tree,"JetPuppi_puId_frac04")
         Prompt_majW=getattr(sig_tree,"JetPuppi_puId_majW")
         Prompt_minW=getattr(sig_tree,"JetPuppi_puId_minW")
         Prompt_jetR=getattr(sig_tree,"JetPuppi_puId_jetR")
         Prompt_nConstituents=getattr(sig_tree,"JetPuppi_nConstituents")
         Prompt_ptD=getattr(sig_tree,"JetPuppi_puId_ptD")
         Prompt_pull=getattr(sig_tree,"JetPuppi_puId_pull")
         Prompt_Rho=getattr(sig_tree,"fixedGridRhoFastjetAll")
         if eta_bin!="Eta3p0To5p0":
            Prompt_Charged=getattr(sig_tree,"JetPuppi_puId_nCharged")
            Prompt_jetRchg=getattr(sig_tree,"JetPuppi_puId_jetRchg")
            Prompt_beta=getattr(sig_tree,"JetPuppi_puId_beta")
      ...
      for entry in range(bkg_tree.GetEntries()): 
         ...
         Pileup_dR2Mean=getattr(bkg_tree,"JetPuppi_puId_dR2Mean")
         Pileup_frac01=getattr(bkg_tree,"JetPuppi_puId_frac01")
         Pileup_frac02=getattr(bkg_tree,"JetPuppi_puId_frac02")
         Pileup_frac03=getattr(bkg_tree,"JetPuppi_puId_frac03")
         Pileup_frac04=getattr(bkg_tree,"JetPuppi_puId_frac04")
         Pileup_majW=getattr(bkg_tree,"JetPuppi_puId_majW")
         Pileup_minW=getattr(bkg_tree,"JetPuppi_puId_minW")
         Pileup_jetR=getattr(bkg_tree,"JetPuppi_puId_jetR")
         Pileup_nConstituents=getattr(bkg_tree,"JetPuppi_nConstituents")
         Pileup_ptD=getattr(bkg_tree,"JetPuppi_puId_ptD")
         Pileup_pull=getattr(bkg_tree,"JetPuppi_puId_pull")
         Pileup_Rho=getattr(bkg_tree,"fixedGridRhoFastjetAll")
         if eta_bin!="Eta3p0To5p0":
            Pileup_Charged=getattr(bkg_tree,"JetPuppi_puId_nCharged")
            Pileup_jetRchg=getattr(bkg_tree,"JetPuppi_puId_jetRchg")
            Pileup_beta=getattr(bkg_tree,"JetPuppi_puId_beta")
   ...
   if eta_bin!="Eta3p0To5p0":
      signal=[s_npvs,s_dR2Mean,s_frac01,s_frac02,s_frac03,s_frac04,s_majW,s_minW,s_jetR,s_nConstituents,s_ptD,s_pull,s_Rho,s_beta,s_jetRchg,s_Charged]
      background=[b_npvs,b_dR2Mean,b_frac01,b_frac02,b_frac03,b_frac04,b_majW,b_minW,b_jetR,b_nConstituents,b_ptD,b_pull,b_Rho,b_beta,b_jetRchg,b_Charged]
   if eta_bin=="Eta3p0To5p0":
      signal=[s_npvs,s_dR2Mean,s_frac01,s_frac02,s_frac03,s_frac04,s_majW,s_minW,s_jetR,s_nConstituents,s_ptD,s_pull,s_Rho]
      background=[b_npvs,b_dR2Mean,b_frac01,b_frac02,b_frac03,b_frac04,b_majW,b_minW,b_jetR,b_nConstituents,b_ptD,b_pull,b_Rho]
   
   if eta_bin!="Eta3p0To5p0":
      var=["npvs","dR2Mean","frac_01","frac_02","frac_03","frac_04","majW","minW","jetR","nConstitunents","ptD","pull","Rho","beta","jetRchg","nCharged"]
   if eta_bin=="Eta3p0To5p0":
   ...
   for i,name in enumerate(var):
      ...
      lgd.AddEntry(signal[i],"Signal")
      lgd.AddEntry(background[i],"Background")
      ...
      if sig_max <= bkg_max:
         ...
         background[i].Draw("HIST SAME")
      lgd.Draw("SAME")
      c.SaveAs("GenIdx_plot_%s_%s.png" %(name,eta_bin))
