...
#        if event.JetPuppi_pt[k] > f_maxJetpt or event.JetPuppi_pt[k] < f_minJetpt or Gen_index == -1: continue
        ...
        if (dRMatch_ <= 0.2 and GenJetIdx < event.nGenJet and GenJetIdx >= 0 and event.JetPuppi_puId_jetRchg[k]<=0.5 and event.JetPuppi_puId_jetRchg[k]>0.35 and event.JetPuppi_puId_nCharged[k]==0):
#        if (GenJetIdx < event.nGenJet and GenJetIdx >= 0):
            isPrompt = True
            nPrompt += 1
#        if (dRMatch_ >= 0.4 and abs(flavor_) == 0 and GenJetIdx < 0):
###################selecting GenJetIdx already has 0.4 cut
        if (abs(flavor_) == 0 and GenJetIdx < 0 and event.JetPuppi_puId_jetRchg[k]<=0.5 and event.JetPuppi_puId_jetRchg[k]>0.35 and event.JetPuppi_puId_nCharged[k]==0):
#        if (GenJetIdx < 0):
            isPileup = True
            nPileup += 1
        key = ""
