...
eta_bins=["Eta0p0To2p5"]
eta_bin_range=["[0.0,2.5]"]
#input_files=[]
pre_input_files=[]
post_input_files=[]
for idx in range(48):
   inputfile="input/EE_Comparison_training_trees_pt10To100_puppi_130X_%i_MC22EE.root" %idx
   post_input_files.append(inputfile)

for idx in range(13):
   inputfile="input/EE_Comparison_training_trees_pt10To100_puppi_130X_%i_MC22.root" %idx
   pre_input_files.append(inputfile)
...
for x, eta_bin in enumerate(eta_bins):
   ...
   s_eta=ROOT.TH1D("Jet_Eta","Jet #eta #eta #in %s" %eta_bin_range[x],20,1.2,2.4)
   ...
   b_eta=ROOT.TH1D("Jet_Eta","Jet #eta #eta #in %s" %eta_bin_range[x],20,1.2,2.4)
...
#      background[i].
      ...
      lgd.AddEntry(signal[i],"pre_EE")
      lgd.AddEntry(background[i],"post_EE")
      c=ROOT.TCanvas("canvas","canvas",500,700)
      ratio = signal[i].Clone()
      den_ratio = background[i].Clone()
      ratio.Scale(1/ratio.Integral())
      den_ratio.Scale(1/den_ratio.Integral())
      
      ratio.Divide(den_ratio)
      ...
      signal[i].GetXaxis().SetLabelSize(0)
      signal[i].GetXaxis().SetTitleSize(0)
      background[i].GetXaxis().SetLabelSize(0)
      background[i].GetXaxis().SetTitleSize(0)
      ...
      if sig_max>bkg_max:
         signal[i].DrawNormalized("HIST")
         background[i].DrawNormalized("HIST SAME")
      if sig_max <= bkg_max:
         background[i].DrawNormalized("HIST")
         signal[i].DrawNormalized("HIST SAME")
         background[i].DrawNormalized("HIST SAME")
      lgd.Draw("SAME")
      ...
      pad2.cd()
      ratio.Draw("HIST")
      c.SaveAs("Prompt_EE_Comparison_RUN3_plot_%s_%s.png" %(name,eta_bin))
